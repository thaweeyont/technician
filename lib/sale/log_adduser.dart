...
import 'package:technician/models/product_head_data_all_model.dart';
...
class Log_Adduser extends StatefulWidget {
  ...
  Log_Adduser(
      this.id_head_job, this.idcard, this.name_u, this.phone_u, this.address_u);

  @override
  _Log_AdduserState createState() => _Log_AdduserState();
}

enum SingingCharacter { morning, evening }

class _Log_AdduserState extends State<Log_Adduser> {
  var id_staff, name_staff, id, edit_amount;
  ...
  List<ProductHeadDataAll> dataall = [];
  ...
  void initState() {
    ...
    get_provinces();
    setState(() {
      idcard.text = widget.idcard;
      fullname.text = widget.name_u;
      phone.text = widget.phone_u;
      address.text = widget.address_u;
    });
    CheckPermission();
    getprofile_staff();
    _check_user(widget.idcard);
    if (widget.id_head_job != "") {
      _getProduct(widget.id_head_job);
    }
    _getdataall(widget.id_head_job);
  }
  ...
  Future next_page(String status_function) async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(context, CupertinoPageRoute(builder: (context) {
        return Addlocation(widget.id_head_job, idcard.text, lat!, lng!);
      }));
    } else {
      print("ไม่สำเร็จ");
    }
    ...
    try {
      ...
      if (respose.statusCode == 200) {
        setState(() {
          ...
          dropdownValue = productjob[0].productTypeContract!;
        });
      } else {
        setState(() {
          show_product = 0;
  ...
  //เรียกใช้ api แสดง ข้อมูลทั้งหมด
  Future<Null> _getdataall(String gen_id_job) async {
    try {
      var respose = await http.get(Uri.http(
          ipconfig,
          '/flutter_api/api_staff/get_data_job_head.php',
          {"gen_id_job": gen_id_job}));
      // print(respose.body);
      if (respose.statusCode == 200) {
        setState(() {
          dataall = productHeadDataAllFromJson(respose.body);
          dateinput.text = dataall[0].dateinput!;
          var val_date = dataall[0].dateGotoSt;
          if (val_date == "เช้า") {
            _character = SingingCharacter.morning;
            date_goto_st = "เช้า";
          } else {
            _character = SingingCharacter.evening;
            date_goto_st = "บ่าย";
          }
        });
      }
    } catch (e) {
      print("ไม่มีข้อมูลทั้งหมด");
    }
  }
                                ...
                                children: [
                                  ...
                                  Container(
                                    child: DropdownButton(
                                      ...
                                      items:
                                          product_type_list.map((productlist) {
  ...
  Future<Null> delete_product(String id, String name) async {
    showAnimatedDialog(
      context: context,
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              onPressed: () async {
                                if (edit_selectedValue_product == null ||
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      'กรุณาเพิ่มสินค้า',
                      style: MyConstant().normallightStyle(),
                    ),
